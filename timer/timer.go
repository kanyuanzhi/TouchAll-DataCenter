...
func (t *Timer) Run() {
	for f := range t.funcChan {
		...
		case "day":
			utils.StartTimerByDay(*f, t.funcToInfo[f].hour, t.funcToInfo[f].min, t.funcToInfo[f].sec)
		case "month":
			utils.StartTimerByMonth(*f, t.funcToInfo[f].date)
		case "interval":
			utils.StartTimerByInterval(*f, t.funcToInfo[f].interval)
		default:
			continue
		}
	}
